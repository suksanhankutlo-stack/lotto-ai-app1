# ============================================================
# 🤖 LOTTO AI PRO V8.1 MAX (REFACTORED: TUNED, FAST & ROBUST)
...
# ============================================================
# 2. DATE & ROBUST SCRAPER (รองรับ 6 หลัก หวยไทย)
# ============================================================
...
def fetch_lottery_data(url):
    ...
    try:
        ...
        text_elements = [el.get_text(" ", strip=True) for el in content.find_all(["tr", "p", "div", "li", "span"]) if el.get_text(" ", strip=True)]
        if not text_elements: text_elements = content.get_text(separator="\n").split("\n")

        for text in text_elements:
            if parsed_date := normalize_date(text): current_date = parsed_date
            
            # ค้นหาทั้ง 6 หลักและ 3 หลัก
            numbers6 = re.findall(r"\b\d{6}\b", text)
            numbers3 = re.findall(r"\b\d{3}\b", text)
            numbers2 = re.findall(r"\b\d{2}\b", text)
            
            if current_date and numbers6 and numbers2:
                extracted.append({"Date": current_date, "Result_3D": numbers6[0][-3:], "Result_2D": numbers2[-1]})
            elif current_date and numbers3 and numbers2:
                extracted.append({"Date": current_date, "Result_3D": numbers3[0], "Result_2D": numbers2[-1]})

        if not extracted:
            for line in content.get_text(separator="\n").split("\n"):
                if p_date := normalize_date(line): current_date = p_date
                if match := re.search(r"\b(\d{6})\b.*?\b(\d{2})\b", line):
                    if current_date: extracted.append({"Date": current_date, "Result_3D": match.group(1)[-3:], "Result_2D": match.group(2)})
                elif match := re.search(r"\b(\d{3})\b.*?\b(\d{2})\b", line):
        ...
        df = df.dropna().query("Result_3D.str.match('^\\d{3}$') and Result_2D.str.match('^\\d{2}$')").drop_duplicates(subset=["Date", "Result_3D", "Result_2D"]).sort_values("Date").reset_index(drop=True)
...
# ============================================================
# 4. CONFIG & MODELS (SPEED OPTIMIZED)
# ============================================================
...
def strict_walk_forward_backtest(df_feat, pos, config):
    ...
    for t_idx in range(start, n):
        if t_idx % 5 == 0: cached_features = None # Cache refresh speedup
